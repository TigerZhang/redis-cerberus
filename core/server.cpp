...
void Server::_read_response()
{
    ...
    if (responses.size() > this->_sent_commands.size()) {
        ...
        for (util::unique_pointer<Response> const& rsp: responses) {
...
void Server::pop_client(Client* cli)
{
    util::erase_if(
        ...
        [&](util::weak_pointer<DataCommand> cmd)
        {
            return cmd->group->client.is(cli);
        });
    for (util::weak_pointer<DataCommand>& cmd: this->_sent_commands) {
...
std::vector<util::weak_pointer<DataCommand>> Server::deliver_commands()
{
    util::erase_if(
        this->_sent_commands,
        [](util::weak_pointer<DataCommand> cmd)
...
void Server::close_conn()
{
    if (!this->closed()) {
        ...
        for (util::weak_pointer<DataCommand> c: this->_sent_commands) {
            if (c.nul()) {
                continue;
            }
            this->_proxy->retry_move_ask_command_later(c);
...
static std::function<void(int, std::vector<util::weak_pointer<DataCommand>>&)> on_server_connected(
    [](int, std::vector<util::weak_pointer<DataCommand>>&) {});
...
{
    ::on_server_connected =
        [](int fd, std::vector<util::weak_pointer<DataCommand>>& cmds)
        {
            flush_string(fd, READONLY_CMD);
            cmds.push_back(util::weak_pointer<DataCommand>(nullptr));
        };
}

bool Server::need_try_to_promote_from_db(util::weak_pointer<DataCommand> command) {
...
void Server::try_to_promote(util::weak_pointer<DataCommand> command, Server *server) {
